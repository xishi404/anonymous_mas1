...
"""
    ...
    async def code_fill(
        ...
        prompt = context
        if return_usage:
            content, tokens = await self.llm.aask_with_usage(prompt, timeout=timeout)
            extracted_code = sanitize(code=content, entrypoint=function_name)
            result = {field_name: extracted_code, "_usage_tokens": tokens}
        else:
    ...
    async def single_fill(self, context: str, images: Optional[Union[str, list[str]]] = None, return_usage: bool = False) -> Dict[str, Any]:
        ...
        prompt = context
        if return_usage:
            content, tokens = await self.llm.aask_with_usage(prompt, images=images)
            result = {field_name: content, "_usage_tokens": tokens}
        else:
    ...
    async def xml_fill(self, context: str, images: Optional[Union[str, list[str]]] = None, return_usage: bool = False) -> Dict[str, Any]:
        ...
        if return_usage:
            content, tokens = await self.llm.aask_with_usage(context, images=images)
        else:
        ...
        if return_usage:
            extracted_data["_usage_tokens"] = tokens
    ...
    async def fill(
        ...
        if mode == FillMode.CODE_FILL.value:
            ...
            if return_usage:
                # Store usage tokens as an attribute on the ActionNode
