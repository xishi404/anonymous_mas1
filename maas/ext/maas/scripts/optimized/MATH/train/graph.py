...
class MATHGraph:
    ...
    async def __call__(self, problem: str, log_path: str = None):
        ...
        programmer_latency = time.time() - programmer_start

        # Track initial programmer call
        if "Programmer" not in operator_latencies:
            operator_latencies["Programmer"] = []
            operator_iterations["Programmer"] = []
        operator_latencies["Programmer"].append(programmer_latency)
        operator_iterations["Programmer"].append(1)
        total_operator_time += programmer_latency
